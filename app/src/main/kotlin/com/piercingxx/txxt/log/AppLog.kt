 ...
 * phone numbers, or attachment bytes in [message].
 *
 * Init from [com.piercingxx.txxt.TxxtApp] before anything else. Unit tests
 * call [init] with a temp directory and no logcat sink.
 */
object AppLog {
    ...
    const val LOG_NAME = "txxt.log"
    const val PREV_NAME = "txxt.log.1"
    ...
    private val writer = Executors.newSingleThreadExecutor { r ->
        Thread(r, "txxt-log").apply { isDaemon = true }
    }
    ...
    /** Test-only: drop state so the next [init] takes effect. */
