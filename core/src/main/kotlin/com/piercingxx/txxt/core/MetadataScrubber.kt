 ...
 *
 * [scrub] inspects the leading bytes to dispatch to the image scrubber
 * ([ImageMetadataScrubber]) for JPEG input or the video scrubber
 * ([VideoMetadataScrubber]) for MP4/MOV input, and returns unrecognized input
 * unchanged (there is no recognized metadata container to strip). Like the
...
object MetadataScrubber {
    ...
    /**
     * Returns a copy of [media] with privacy metadata containers stripped.
     *
     * JPEG input has its EXIF/XMP/IPTC APP segments removed; MP4/MOV input has
     * its GPS/device/creation-time/encoder atoms removed. The media bytes
     * themselves are never re-encoded — only the metadata containers are
     * dropped (see [ImageMetadataScrubber] and [VideoMetadataScrubber]).
     * Input that is neither a JPEG nor an MP4/MOV is returned unchanged.
     *
